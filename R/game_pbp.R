#' @rdname game_pbp
#' @title **Acquire pitch-by-pitch data for LMB and LMP games**
#'
...
#' @export
#' @examples \donttest{
#'   try(game_pbp(game_pk = 632970))
#' }

game_pbp <- function(game_pk) {
  empty_df <- data.frame(game_pk=character(),game_date=character(),index=character(),startTime=character(),
                         endTime=character(),
                         isPitch=character(),
                         type=character(),
                         playId=character(),
  ...
  tryCatch(
    expr = {
      ...
      pbp <- dplyr::bind_rows(empty_df, pbp)
      ...
      pbp <- pbp %>%
        ...
        make_baseballmexR_data(paste0(game_pk, " Game data from MLB.com"),Sys.time())
    },
    error = function(e) {
